 ...
 * @return                      0 on success; nonzero on failure.
 */
int boot_scramble_region(const struct flash_area *fa, uint32_t off, uint32_t size,
                         bool backwards) {
    int rc = 0;
...
fih_ret /**/context_boot_go(struct boot_loader_state* state, struct boot_rsp* rsp) {
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        #ifdef MCUBOOT_VALIDATE_PRIMARY_SLOT
        FIH_CALL(boot_validate_slot, fih_rc, state, BOOT_PRIMARY_SLOT, NULL, 0);    /* MCUBOOT_SEQ15 */
