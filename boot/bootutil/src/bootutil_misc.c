 ...
 */
struct flash_area const* boot_find_status(const struct boot_loader_state* state, int image_index) {
    struct flash_area const* fa_p = NULL;
    struct flash_area const* areas[] = {
        #if MCUBOOT_SWAP_USING_SCRATCH
        state->scratch.area,
        #endif
