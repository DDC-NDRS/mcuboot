 ...
 * progress itself.
 */
static inline uint32_t boot_trailer_info_sz(void) {
    return (
        #ifdef MCUBOOT_ENC_IMAGES
        /* encryption keys */
 ...
 * operation.
 */
static inline uint32_t boot_status_entry_sz(uint32_t min_write_sz) {
...
#ifdef MCUBOOT_ENC_IMAGES
int boot_read_enc_key(const struct flash_area* fap, uint8_t slot, struct boot_status* bs) {
    uint32_t off;
    #if MCUBOOT_SWAP_SAVE_ENCTLV
    int i;
    #endif
...
int boot_write_swap_size(const struct flash_area* fap, uint32_t swap_size) {
    ...
    return boot_write_trailer(fap, off, (uint8_t const*)&swap_size, 4);
}
